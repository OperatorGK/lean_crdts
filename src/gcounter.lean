...
def new (n: ℕ) : gcounter n := { data := λ _, 0 }
...
lemma value_new_aux₁ {n: ℕ} : (new n).to_list = list.repeat 0 n :=
begin
	apply list.ext_le,
	simp only [array.to_list_length, list.length_repeat],

	intros,
	rw [list.nth_le_repeat],
	rw [list.length_repeat] at h₂,

	rw array.to_list_nth_le,
	simp only [new, array.read, d_array.read],
	assumption,
end

lemma value_new_aux₂ {n: ℕ} : list.foldl (+) 0 (list.repeat 0 n) = 0 :=
begin
	induction n,
	simp only [list.repeat, list.foldl, add_zero],
	assumption,
end

lemma value_new {n: ℕ} : value (new n) = 0 :=
begin
	simp only [value, ← array.to_list_foldl, add_comm, value_new_aux₁, value_new_aux₂],
end
